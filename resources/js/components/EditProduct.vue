...
export default {
    ...
    methods: {
        ...
        updateProduct() {
            ...
            axios.post('/product-update', product).then(response => {
        ...
        file_removed(file, error, xhr){
            console.log(file, error, xhr);
            if(file.hasOwnProperty('id')){

            }
            axios.post('/product-image-delete', {id:file.id}).then(response => {
                console.log(response.data);
    ...
    created(){
        ...
        this.product_variant  = this.productVariants;
        this.product_variant_prices  = this.productVariantPrices;        
    },
    mounted() {
        var self = this;
        this.productImages.forEach(function(item){
            let file = { size: 123, name: "test", type: "image/png",id: item.id, };
            let url = item.url;
